...
### 1. 레이아웃의 기본 개념(200p)
>* 레이아웃
>   * ViewGroup클래스로부터 상속받으며 내부에 무엇을 담는 용도
>   * 즉, 레이아웃 안에 존재하는 위젯을 배치하게 해줌.
>   * LinerLayout : 레이아웃에서 가장 많이 사용됨. 선형 레이아웃
>
>* 레이아웃에서 자주 사용되는 속성
>   * 
>   * 
...
>
>* 렐러티브레이아웃(상대 레이아웃)
>   * 위젯 자신이 속한 레이아웃의 상하 좌우의 위치를 지정하여 뱇
>   * 다른 위젯으로부터 상대적인 위치 지정  ex)A위젯의 오른쪽에 있다
>
>* 테이블레이아웃
>   * 행과 열의 개수를 지정한 테이블 형태로 위젯을 배열한다.
>
>* 그리드레이아웃
>   * 테이블레이아웃과 비슷하지만, 행 또는 열을 확정하여 다양하게 배치할 때 더 편리함.
>   
>* 프레임레이아웃
>   * 위젯들을 왼쪽 위에 일률적으로 겹쳐서 배치하며, 중복해서 보이는 효과를 냄.
>   * 같은 위치에, 선택사항에 따라 다른 위젯을 보여줘야 할 때 사용함. (보이고 안 보이고 이렇게 할 때 자주 쓰임.
>   * 주로 리니어레이아웃 안에서 중첩으로 사용함(단독으로는 잘 사용x)

## 02. 리니어레이아웃(203p)
### 1. 기본 리니어아웃의 형태
>* **orientation 속성** [필수!!!]
>   * 리니어레이아웃의 가장 기본적인 속성
>   * vertical : 리니어레이아웃 안에 포함될 위젯의 배치를, 왼쪽 위부터 수직방향으로 쌓음.
>   * horizontal : 수평방향으로 쌓음.
>   
>* gravity속성 (리니어레이아웃 안에 있는 속성)
>   * 자신에게 포함된 자식(주로 위젯)을 어디에 배치할 것인지를 결정함.
>   
>* layout_gravity속성
>   * 자신의 위치를 부모(주로 레이아웃)의 어디에 위치시킬지를 결정함. 

## 03. 기타 레이아웃(217p)
### 1. 프레임레이아웃(232p)
>* 프레임레이아웃
>   * 단순히 레이아웃 안의 위젯을 왼쪽 상단으부터 겹쳐서 출력함.

# 6장 고급 위젯 다루기
## 01. 고급 위젯
### 1. 날짜와 시간 관련 위젯
>* 아날로그시계와 디지털시계
>   * 화면에 시간을 표시하는 위젯. 시계를 표현하는 용도로 쓰임.
>   * 아날로그시계(AnalogClock)
>   * 디지털시계(DigitalClock)
>   
>* 크로노미터(Chronometer)
>   * 타이머 형식의 위젯.
>   * 일반적으로 시간을 측정할 때 많이 사용.(스톱워치)
>
>* 타임피커(TimePicker)
>   * 시간을 표시하고 조절하는 기능
>
>* 데이트피커(DatePicker), 캘린더뷰(CalendarView)
>   * 날짜를 표시하고 조절하는 기능
>
>* [실습 6-1 날짜/시간 예약 앱 만들기](243~249p)
>* setOnDateChange
>   OnSelectedDateChange
>* 자바는 0~11 까지 월을 나타냄. 그래서 +1을 해줘야 원하는 월(Month)을 표현할 수 있다.

//수업 끝나도, 책 보면서 하나하나 해보기!
# 7장 메뉴와 대화상자(284p)
## 01. 메뉴
### 1. 
## 02.
...
### 2. 이미지의 기하학적 변화
### 3. 이미지 활용
## 06. 액티비티와 인텐트의 기본[14장에도 간단히 나옴. 따라해볼 것!]
>* **인텐트** : 화면간의 데이터를 주고 받을 때 사용하는 도구.
### 1. 안드로이드의 4대 컴포넌트
>* 액티비티
>* 서비스
>* 브로드캐스트 리시버
...
>* 콘텐트 프로바이터
### 2. 
## 07. 어뎁터 뷰[10장?9장에 있음. 따라해볼 것!]
### 1. 리스트 뷰
>* 최신통화목록
>* 전화목록
>* 이런 형태로 쓰이는게 리스트 뷰 라는 위젯.
### 2. 그리드 뷰
### 2. SQLLIte(미니 데이터베이스?)
### 1. 오디오
[실습 13-1 간단한 MP3 플레이어 만들기]
### 3. UI 스레드
### 4. 스레드의 응용
>* [예제13-3의 세번째 레이아웃인 19~26행 수정]
# 10장 구글 지도
## 01. 구글 지도 사용을 위한 준비
>* 어떤걸 반드시 인스톨 해야 함.
>* 그리고 인증키도 요청 해야 함.
...
>교재만 완료 해보기! 그러면 어느 정도는 파악하고 이해 될 것임.
>요즘 It개발자 인기 많음
>개발한지 1~2년 지난 개발자가 인기 많음
>시키는대로 잘 하고, 성실하고, 지식도 있고
>근데 머리가 더 커지면 안 좋아함. 지멋대로 구니깐.
>신입은 어디서든 시작해야 함.
>일이 힘들고. 돈을 조금 줘도. 분위기가 좋은 회사가 있다. 그런 회사에 가서 경력을 쌓아야 한다.
>프로그램 개발자는 되게 평등. 위 아래가 공평, 평등. 본인잉 해야할 거 팀 내에서 잘 하면 됨.
>돈, 작업환경 보다. 팀 내 분위기나 사람들 간의 그런것을 보고 회사를 가는게 나음. 힘들어도 1~2년은 다니는게 낫다.
>길게 봐라.
